...
In \citep{DAFX} Moorer suggests adding an early reflection network before the six \gls{lpcf}'s. The early reflection network is a simple multi tap \gls{fir} filter constellation. Moorer also suggests making the \gls{lpcf} feedback gain as a variable, so the user can change the room size and damping. The following \autoref{eq:reverb_eq_eks} shows the differential equation for one of the \gls{lpcf}'s \citep{LPCFfd}. 
