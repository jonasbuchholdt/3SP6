...
The equalizer is implemented using 16-bits precision and the main algorithm of the amplifying part of the equalizer is shown in \autoref{}. 

\todo[inline]{Insert filter code when done}

The code shows the implementation of one filter as for example the filter represented in \autoref{eq:z_to_n7}. Line XX does the calculation $- \frac{b_{peak3}}{b_{peak1}} \cdot y_{peak}[n-2]$ and stores it in AC0. Line XX does the calculation $-  \frac{b_{peak2}}{b_{peak1}} \cdot y_{peak}[n-1]$, adds it to what is already in AC0, and moves $y_{peak}[n-1]$ to $y_{peak}[n-2]$. This way \autoref{eq:z_to_n7} does the calculation of $y_{peak}[n]$. In line XX $y_{peak}[n]$ is stored in $y_{peak}[n-1]$ to be ready to the next sample.  
Line XX is special for the amplifying filters, because the coefficient in this line is zero, because \autoref{eq:z_to_n7} does not use $x[n-1]$ in its calculations. 
