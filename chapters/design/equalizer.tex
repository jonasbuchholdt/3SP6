...
The final equalizer should be able to both amplify and attenuate each band. Therefore the gain values for attenuating a band must also be calculated. The gain for attenuating a peak filter \SI{15}{\decibel} is found in \autoref{}

\begin{subequations}
\begin{equation}\label{eq:peak_gain_at}
       20 \cdot \log_{10} \left(\Big|\frac{1}{1 + H_{peak}(j\omega_0)}\Big|\right) = 20 \cdot \log_{10} \left(\frac{1}{1+G_{peak-cut}}\right) = 15 \addunit{\SI{}{\decibel}}
    \end{equation}
\centering
$\Updownarrow$
\begin{equation}\label{eq:peak_gain_value_at}
        G_{peak-cut} = -1+10^{\frac{-15}{20}} = -0.49 \addunit{\SI{}{1}}
    \end{equation}
 \end{subequations}

In the final gain for the attenuation, the gain has to be added with one, thus $G_{peak-cut} = -0.49 + 1 = 0.51$ 
