...
;\end{circuitikz}
\caption{The equalent input schematic of $G1$ \autoref{fig:simple_preamp}}
...
\begin{equation}\label{eq:preamp_amplification}
 ...
 \subsection{Design of $G1$ DC offset}
  ...
  Designing of the voltage offset power supply will be done by two resistors which divide the voltage by two, and afterwards an \gls{opamp} in buffer circuit is used to make a small output impedance. Because the \gls{opamp} have a impedance of more than \SI{1}{\mega\ohm} the resistor size does not matte at all, but an \gls{opamp} do have a bias current in the input, that will affect the division if the resistors is to large. To keep the division approximate by two the resistors shall be at leas a decade smaller than the input impedance of the \gls{opamp}. The voltage offset power supply circuit is designed as following \autoref{fig:preamp_voffset}.
...
\begin{equation}\label{eq:preamp_offset}
    ...
    \startexplain
        ...
        \explain{$R_3$ is the lower resistor in the voltage division circuit}{\si{\ohm}}
    \stopexplain
    
\subsection{Design of input and output DC blocking}

The total circuit of the \gls{preamp} is as \autoref{fig:preamp_total} and the design of $G1$ input $C_{In}$ and  $G2$ output $C_{Out}$ will done in this section.
...
to[R=$R_{Bias}$] (-3,0.5)
to[short](opamp1.+) 
(-5,0.5)to[C=$C_{In}$, o-] (-3,0.5)%node[left,above]{$Input$}
(opamp1.-) 
(opamp1.out) 
to[short] (2,0)
to[short] (2,-1.5)
...
;\end{circuitikz}
\caption{The schematic over}
\label{fig:preamp_total}
\end{figure}

The $C_{In}$ and $Z_{In}$ shall be designed to have an cross frequency of a decade beneath the audibility area, which will result in a cross frequency of \SI{2}{\hertz}. The calculating of $C_{In}$ is done by following \autoref{eq:preamp_cin}.

\begin{equation}\label{eq:preamp_cin}
        C_{In} = \frac{1}{2 \pi \cdot Z_{In} \cdot f}
    \end{equation}

    \startexplain
         \explain{$C_{In}$ is the Input capacitor}{\si{\farad}}
        \explain{$Z_{In}$ is the input impedance of $G1$}{\si{\ohm}}
        \explain{$f$ is the cross frequencyt}{\si{\hertz}}
    \stopexplain
    
    The $C_{Out}$ and $Z_{Out}$ shall be designed to have an cross frequency of a decade over the audibility area, which will result in a cross frequency of \SI{200}{\kilo\hertz}. The calculating of $C_{Out}$ is done by following \autoref{eq:preamp_cout}.

\begin{equation}\label{eq:preamp_cout}
        C_{In} = \frac{1}{2 \pi \cdot Z_{Out} \cdot f}
    \end{equation}
